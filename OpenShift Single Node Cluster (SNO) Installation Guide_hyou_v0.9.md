...
### 17. Time 서버 및 Timezone 설정

master 서버의 시간 및 Timezone을 설정합니다. bastion 서버에 구성된 chrony 서버를 바라보도록 설정하고, Timezone을 KST로 변경할 수 있습니다.

RHEL 8 버전의 경우에는 Butane을 통해 파일 내용을 인코딩하여 포함할 수 있는 툴을 제공합니다. RHEL 7 버전의 경우에는 다음과 같이 작업하여 적용 할 수 있습니다.

**17-1) Time 서버 설정**

- `chrony.conf` 내용을 생성하여 base64로 인코딩 합니다.

  ```bash
  cat << EOF | base64
      pool bastion.single.demo.com iburst iburst 
      driftfile /var/lib/chrony/drift
      makestep 1.0 3
      rtcsync
      logdir /var/log/chrony
...
- Example Output
  ...
  ```bash
  ICAgIHBvb2wgYmFzdGlvbi5zaW5nbGUuZGVtby5jb20gaWJ1cnN0IGlidXJzdCAKICAgIGRyaWZ0
  ZmlsZSAvdmFyL2xpYi9jaHJvbnkvZHJpZnQKICAgIG1ha2VzdGVwIDEuMCAzCiAgICBydGNzeW5j
  CiAgICBsb2dkaXIgL3Zhci9sb2cvY2hyb255Cg==
  ```

- chrony 설정 (99-master-chrony.yaml)

  base64 문자열을 방금 생성한 문자열로 교체하여 `MachineConfig` Object 파일을 생성합니다. 

  ```bash
  cat << EOF >> 99-master-chrony.yaml
  ...
  spec:
    config:
      ...
      storage:
        files:
        - contents:
            source: data:text/plain;charset=utf-8;base64,ICAgIHBvb2wgYmFzdGlvbi5zaW5nbGUuZGVtby5jb20gaWJ1cnN0IGlidXJzdCAKICAgIGRyaWZ0ZmlsZSAvdmFyL2xpYi9jaHJvbnkvZHJpZnQKICAgIG1ha2VzdGVwIDEuMCAzCiAgICBydGNzeW5jCiAgICBsb2daXIgL3Zhci9sb2cvY2hyb255Cg==
          mode: 420
          overwrite: true
...
- 파일 적용
  ...
  클러스터가 정상적으로 구성된 이후에 해당 파일을 적용합니다.
...
- Timezone 설정 변경을 위한 `MachineConfig` Object 파일을 생성

  ```yaml
  cat <<EOF> 99-master-timezone.yaml 
  apiVersion: machineconfiguration.openshift.io/v1
  kind: MachineConfig
  metadata:
    labels:
...
- 파일 적용
  ...
  ```bash
  $ oc apply -f 99-master-timezone.yaml 
  ```
